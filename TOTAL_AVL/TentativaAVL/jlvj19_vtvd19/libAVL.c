...
//=========== ExcluiArvore    -> Exclui uma arvore inteira a partir da raiz  ===========//
/* Utiliza o conceito de pós-ordem para deletar corretamente */
void excluiArvore(tNodo *nodo)
{
    if (nodo != NULL)
    {
        excluiArvore(nodo->esquerda);
        excluiArvore(nodo->direita );
        free(nodo);
    }
}